...
class ChatConfigCog(commands.Cog):
    ...
    async def toggleactive(self, ctx, persona: discord.app_commands.Choice[str] = None):
        default_persona = instructions.get(instruc_config)
        if persona is None:
            class FakeChoice:
                def __init__(self, value):
                    self.value = value
            persona = FakeChoice(default_persona)
